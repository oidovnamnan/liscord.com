...
import { Loader2, Save, MessageSquare, Shield, AlertTriangle, Sun, Moon, Monitor, Cpu, Settings, Sparkles } from 'lucide-react';
...
export function SuperAdminGlobalSettings() {
    ...
    return (
        <div className="page-container animate-fade-in" style={{ padding: '24px clamp(16px, 3vw, 32px) 32px' }}>
            {/* ── Premium Hero ── */}
            <div className="sa-hero" style={{ background: 'linear-gradient(135deg, #334155 0%, #475569 40%, #6366f1 100%)', boxShadow: '0 8px 32px rgba(99, 102, 241, 0.2)' }}>
                <div className="sa-hero-top">
                    <div className="sa-hero-left">
                        <div className="sa-hero-icon"><Settings size={24} /></div>
                        <div>
                            <div className="sa-hero-badge"><Sparkles size={10} /> Глобал тохиргоо</div>
                            <h1 className="sa-hero-title">Системийн Тохиргоо</h1>
                            <div className="sa-hero-desc">Бүх хэрэглэгчид харагдах мэдэгдэл болон ерөнхий төлөв</div>
                        </div>
                    </div>
                    <button
                        className="sa-hero-btn"
                        onClick={handleSaveClick as any} // eslint-disable-line @typescript-eslint/no-explicit-any
                        disabled={saving}
                    >
                ...
                <div className="sa-hero-stats">
                    <div className="sa-hero-stat">
                        <div className="sa-hero-stat-value" style={{ fontSize: '1rem' }}>{theme === 'light' ? '☀️ Цайвар' : theme === 'dark' ? '🌙 Бараан' : '🖥️ Систем'}</div>
                        <div className="sa-hero-stat-label">Загвар</div>
                    </div>
                    <div className="sa-hero-stat">
                        <div className="sa-hero-stat-value" style={{ fontSize: '1rem' }}>{settings.banner.isActive ? '🟢 Асаалттай' : '⚪ Унтарсан'}</div>
                        <div className="sa-hero-stat-label">Banner</div>
                    </div>
                    <div className="sa-hero-stat">
                        <div className="sa-hero-stat-value" style={{ fontSize: '1rem' }}>{settings.registrationEnabled ? '🟢 Нээлттэй' : '🔴 Хаалттай'}</div>
                        <div className="sa-hero-stat-label">Бүртгэл</div>
                    </div>
                    <div className="sa-hero-stat">
                        <div className="sa-hero-stat-value" style={{ fontSize: '1rem' }}>{settings.maintenanceMode ? '🔴 Идэвхтэй' : '🟢 Хэвийн'}</div>
                        <div className="sa-hero-stat-label">Maintenance</div>
                    </div>
                </div>
            </div>
            ...
            <form onSubmit={handleSaveClick} style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '800px' }}>
                ...
                <div className="card">
                    <h2 style={{ fontSize: '1.1rem', margin: '0 0 20px 0', display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--text-primary)' }}>
                        <Sun size={20} color="#f59e0b" fill="#f59e0b20" />
                        Системийн Ерөнхий Загвар (Theme)
                    </h2>
                ...
                {/* ── System Banner ── */}
                <div className="card">
                    <h2 style={{ fontSize: '1.1rem', margin: '0 0 20px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <MessageSquare size={20} className="text-primary" />
                        Глобал Анхааруулга (System Banner)
                    </h2>
                    <p style={{ color: 'var(--text-secondary)', marginBottom: '20px', fontSize: '0.88rem' }}>
                        Системийн хэмжээнд бүх хэрэглэгчдэд дээд хэсэгт харагдах мэдэгдэл.
                    </p>
                ...
                {/* ── System Overrides ── */}
                <div className="card">
                    <h2 style={{ fontSize: '1.1rem', margin: '0 0 20px 0', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--danger)' }}>
                        <Shield size={20} />
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        ...
                        <div style={{
                            ...
                            <label className="ios-switch">
                                ...
                                />
                                <span className="ios-slider" style={{ background: settings.maintenanceMode ? 'var(--danger)' : undefined }}></span>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                {/* ── API Settings ── */}
                <div className="card">
                    <h2 style={{ fontSize: '1.1rem', margin: '0 0 20px 0', display: 'flex', alignItems: 'center', gap: '10px' }}>
